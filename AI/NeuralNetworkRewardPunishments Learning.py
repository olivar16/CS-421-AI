...
class AIPlayer(Player):
    ...
    def __init__(self, inputPlayerId):
        ...
        super(AIPlayer,self).__init__(inputPlayerId, "RewardPunishmentsLearning")
    ...
    ##
    # backPropagation
    # Description: Adjusts the weights of the neural network
    #
    # Parameters:
    #   self - The object pointer
    ...
    def backPropagation(self, target, actual):
        ...
        for i in range(0, len(self.hiddenWeightsToLastNode)):
            error = self.hiddenWeightsToLastNode[i] * lastErrorTerm
    ...
    ##
    # neuralNetwork
    # Description: Takes a gamestate and processes it through a neural network
    #
    # Parameters:
    #   self - The object pointer
    ...
    #
    def neuralNetwork(self, currentState):
        ...
        damage = 0.025 * (UNIT_STATS[QUEEN][HEALTH] - enemyQueen.health)

        inputs.append(damage)

        #input 1: queenSafetyValue
        safetyMultiplier = 0

        # keeps track of the number of ants the player has besides the queen
        #input 2: Number of non-queen ants
        numNonQueenAnts = 0.0
        ...
        for ant in playerInv.ants:
            if ant.type == QUEEN:
                ...
                queenSafetyValue = enemyDistFromQueen / maxDist
    ...
    def initializeNeuralNetwork(self):
        ...
        for i in range(0,len(self.inputToHiddenWeights)):
            for j in range(0,len(self.inputToHiddenWeights[i])):
                self.inputToHiddenWeights[i][j] = random.random()


        #initialize biases
        for i in range(0, 10):
            self.biases.append((1, random.random()))
